...
class QueryIntegrationTests: SQLiteTestCase {
  ...
  func test_select_codable_string() throws {
    let table = Table("levels")
    let level = Expression<Level>("level")
    let experience = Expression<Experience>("experience")
    try db.run(
      table.create { builder in
        builder.column(level)
        builder.column(experience)
      })

    try db.run(table.insert(User(level: .high, experience: .expert)))
    let rows = try db.prepare(table)
    let values: [User] = try rows.map({ try $0.decode() })
    XCTAssertEqual(values.count, 1)
    XCTAssertEqual(values[0].level, .high)
    XCTAssertEqual(values[0].experience, .expert)
  }
  ...
  func test_select_codable() throws {
    ...
    let value = TestCodable(
      int: 5, string: "6", bool: true, float: 7, double: 8,
      date: Date(timeIntervalSince1970: 5000), uuid: testUUIDValue, optional: "optional",
      sub: value1)
  ...
  func test_insert_many() throws {
    let id = try db.run(
      users.insertMany([[email <- "alice@example.com"], [email <- "geoff@example.com"]]))
    XCTAssertEqual(2, id)
  }
  ...
  func test_insert_many_encodables() throws {
    ...
    let value1 = TestOptionalCodable(
      int: 5, string: "6", bool: true, float: 7, double: 8,
      date: Date(timeIntervalSince1970: 5000), uuid: testUUIDValue)
    let valueWithNils = TestOptionalCodable(
      int: nil, string: nil, bool: nil, float: nil, double: nil, date: nil, uuid: nil)
  ...
  func test_upsert() throws {
    ...
    let fetchAge = { () throws -> Int? in
      try self.db.pluck(self.users.filter(self.email == "alice@example.com")).flatMap {
        $0[self.age]
      }
    }
    ...
    let nextId = try db.run(
      users.upsert(email <- "alice@example.com", age <- 42, onConflictOf: email))
  ...
  func test_union() throws {
    ...
    let query3 = users.select(users[*], Expression<Int>(literal: "1 AS weight")).filter(
      email == "sally@example.com")
    let query4 = users.select(users[*], Expression<Int>(literal: "2 AS weight")).filter(
    ...
    let orderedIDs = try db.prepare(
      query3.union(query4).order(Expression<Int>(literal: "weight"), email)
    ).map { $0[id] }
