...
enum Level: String, StringValue, Codable {
  case high
  case low

  public static var declaredDatatype = String.declaredDatatype

  public static func fromDatatypeValue(_ stringValue: String) -> Self {
...
enum Experience: String, Value, Codable {
  case beginner
  case expert
...
class QueryTests: XCTestCase {
  ...
  let level = Expression<Level>("level")
  let experience = Expression<Experience>("experience")

  func test_select_enum() {
    assertSQL(
      "INSERT INTO \"levels\" (\"level\", \"experience\") VALUES ('high', '\"expert\"')",
      try! levels.insert(User(level: .high, experience: .expert)))
    assertSQL(
        "SELECT \"level\" FROM \"levels\" WHERE ((\"level\" = 'high') AND (\"experience\" = 'expert'))",
        levels.select(level) .filter(level == .high && experience == .expert))
  }
  ...
  func test_selectDistinct_withExpressions_compilesSelectClause() {
    assertSQL(
      "SELECT DISTINCT \"age\", \"admin\" FROM \"users\"", users.select(distinct: [age, admin]))
  }
  ...
  func test_union_compilesUnionAllClause() {
    assertSQL(
      "SELECT * FROM \"users\" UNION ALL SELECT * FROM \"posts\"", users.union(all: true, posts))
  }
  ...
  func test_join_whenChained_compilesAggregateJoinClause() {
    assertSQL(
      ...
      users.join(posts, on: posts[userId] == users[id]).join(
        categories, on: categories[id] == posts[categoryId])
    )
  }
  ...
  func test_group_withExpressionNameAndHavingBindings_compilesGroupClause() {
    assertSQL(
      "SELECT * FROM \"users\" GROUP BY \"age\" HAVING \"admin\"", users.group(age, having: admin))
    assertSQL(
      "SELECT * FROM \"users\" GROUP BY \"age\" HAVING (\"age\" >= 30)",
      users.group(age, having: age >= 30))
  }
  ...
  func test_with_compilesWithRecursiveClause() {
    ...
    assertSQL(
      "WITH RECURSIVE \"temp\" AS (SELECT * FROM \"users\") SELECT * FROM \"temp\"",
  ...
  func test_with_compilesWithMaterializedClause() {
    ...
    assertSQL(
      "WITH \"temp\" AS MATERIALIZED (SELECT * FROM \"users\") SELECT * FROM \"temp\"",
  ...
  func test_with_compilesWithNotMaterializedClause() {
    ...
    assertSQL(
      "WITH \"temp\" AS NOT MATERIALIZED (SELECT * FROM \"users\") SELECT * FROM \"temp\"",
  ...
  func test_with_columns_compilesWithClause() {
    ...
    assertSQL(
      "WITH \"temp\" (\"id\", \"email\") AS (SELECT * FROM \"users\") SELECT * FROM \"temp\"",
  ...
  func test_insert_many_encodables() throws {
    ...
    let value1 = TestCodable(
      int: 1, string: "2", bool: true, float: 3, double: 4,
      date: Date(timeIntervalSince1970: 0), uuid: testUUIDValue, optional: nil, sub: nil)
    let value2 = TestCodable(
      int: 2, string: "3", bool: true, float: 3, double: 5,
      date: Date(timeIntervalSince1970: 0), uuid: testUUIDValue, optional: "optional", sub: nil)
    let value3 = TestCodable(
      int: 3, string: "4", bool: true, float: 3, double: 6,
      date: Date(timeIntervalSince1970: 0), uuid: testUUIDValue, optional: nil, sub: nil)
  ...
  func test_update_encodable_with_nested_encodable() throws {
    ...
    let extractedJSON = String(
      sql[
        sql.index(
          sql.startIndex, offsetBy: expectedPrefix.count)..<sql.index(
            sql.endIndex, offsetBy: -expectedSuffix.count)
      ])
    let decodedJSON = try JSONDecoder().decode(
      TestCodable.self, from: extractedJSON.data(using: .utf8)!)
  ...
  func test_scalar_returnsScalarExpression() {
    ...
    assertSQL(
      "SELECT \"intOptional\" FROM \"table\"", table.select(intOptional) as ScalarQuery<Int?>)
    assertSQL(
      "SELECT DISTINCT \"int\" FROM \"table\"", table.select(distinct: int) as ScalarQuery<Int>)
    assertSQL(
      "SELECT DISTINCT \"intOptional\" FROM \"table\"",
      table.select(distinct: intOptional) as ScalarQuery<Int?>)
  }
